...
\title{TD1~: accès concurrents aux ressources}
...
Pour chacun d'entre eux vous devez répondre aux questions suivantes.

\begin{enumerate}
\item Est-ce qu'il y a une (ou plusieurs) \emph{race conditions}.
\item Si non, justifier. Si oui, identifiez-les.
\item À l'aide de verrous (type \inlinecode{Mutex} du paquet \inlinecode{sync}), éliminez les \emph{race conditions}.
\item Assurez-vous qu'il ne reste plus de \emph{race conditions}, par exemple en utilisant l'option \inlinecode{-race} de Go.
\item En bonus, pour \file{ex2.go}, vous pourrez réécrire le code en utilisant des canaux à la place des tableaux.
\end{enumerate}
